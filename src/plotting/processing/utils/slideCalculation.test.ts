...
describe("getNoteConfigs", () => {
  ...
  it("Bb3 can be played in positions 1 and 4.86 on a Bb trombone", () => {
    ...
    const relativePositions = getNoteConfigs(
      defaultTrombone,
      note,
      defaultPlayer,
    );
  ...
  it("Eb2 can not be played on a Bb trombone", () => {
    ...
    const configs = getNoteConfigs(defaultTrombone, note, defaultPlayer);
  ...
  it("B2 and D4 can not be played on default trombone", () => {
    ...
    const b2Configs = getNoteConfigs(defaultTrombone, b2, defaultPlayer);
    const d4Configs = getNoteConfigs(defaultTrombone, d4, defaultPlayer);
  ...
  it("Eb2 is in first position on an Eb alto trombone", () => {
    ...
    const configs = getNoteConfigs(ebTrombone, note, defaultPlayer);
  ...
  it("C3 can be played in position 1.02 on a Bb/F trombone", () => {
    ...
    const configs = getNoteConfigs(tromboneBbF, note, defaultPlayer);
  ...
  it("B1 is accesible on a Bb/F trombone with lip bend", () => {
    ...
    const configs = getNoteConfigs(tromboneBbF, note, playerWithLipBend);
    expect(configs).toHaveLength(1);
    const config = configs[0];
  ...
  it("C3 does not require lip bend on a Bb trombone", () => {
    ...
    const configs = getNoteConfigs(defaultTrombone, note, playerWithLipBend);
    expect(configs).toHaveLength(1);
    const config = configs[0];
    expect(config.partial).toBe(3);
  ...
  it("B1 only gets one lip bend config on a Bb/F/E trombone", () => {
    ...
    const configs = getNoteConfigs(tromboneBbFE, note, playerWithLipBend);
    expect(configs).toHaveLength(1);
    const config = configs[0];
    expect(config.partial).toBe(2);
    // Check that slide is maxed out
  ...
  it("Bb2 is in first position on a Bb trombone with first pos distance", () => {
    ...
    const configs = getNoteConfigs(
      tunedDefaultTrombone,
      note,
      playerWithFirstPosDistance,
    );
  ...
  it("B2 is accessible on a Bb trombone with a longer slide length", () => {
    ...
    const configs = getNoteConfigs(longSlideTrombone, note, defaultPlayer);
